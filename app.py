...
def dashboard():
    ...
    if role == "Admin":
        ...
        tasks = c.execute("SELECT * FROM tasks WHERE assigned_to=?", (user_id,)).fetchall()
    ...
    return render_template("dashboard.html", tasks=tasks,
                           total=total, completed=completed, pending=pending, role=role)
...
def create_task():
    ...
    if request.method == "POST":
        ...
        c.execute('''INSERT INTO tasks 
            ...
            VALUES (?,?,?,?,?,?)''',
                  (title, desc, assigned, project, deadline, "Pending"))
